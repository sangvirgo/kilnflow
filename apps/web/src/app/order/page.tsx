...
import { OrderPreview, TraceEvent } from '@kilnflow/shared-types';
...
export default function OrderPage() {
  ...
  const riskBg = preview?.risk.recommend_proceed ? 'bg-green-50' : 'bg-red-50';
  const riskText = preview?.risk.recommend_proceed
    ? 'Khuyen nghi tiep tuc'
    : 'Co rui ro - xem chi tiet:';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div>
        ...
        <button
          ...
          className="mt-2 px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50 text-sm"
        >
          ...
          <div className="border rounded p-4 bg-white">
              ...
              {Object.entries(preview.parsed)
                ...
                .map(([k, v]) => (
                  <tr key={k} className="border-b">
                    <td className="font-mono text-gray-500 pr-2">{k}</td>
            ...
            {preview.parsed.assumptions.length > 0 && (
              <div className="text-xs bg-yellow-50 p-2 rounded mb-2">
                <b>Gia dinh:</b> {preview.parsed.assumptions.join('; ')}
              </div>
            )}
            <div className="text-xs text-gray-500 mb-1">
              Estimation: {preview.estimation.method} ({preview.estimation.confidence})
              \u2014 clay {preview.estimation.estimatedClayKg}kg,
              hours {preview.estimation.estimatedFiringHours}h
              {preview.estimation.basis.length > 0
                ? ', su dung ' + preview.estimation.basis.length + ' me lich su'
                : ''}
            </div>
            <div className={'text-xs p-2 rounded mb-3 ' + riskBg}>
              <b>Risk:</b> {riskText}
              {preview.risk.risks.map((risk, i) => (
