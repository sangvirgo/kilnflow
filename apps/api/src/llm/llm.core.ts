...
export class LlmTransport {
  ...
  async complete(messages: LlmMessage[], opts: LlmCompleteOptions = {}): Promise<string> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        const text = await this.provider.raw(messages, { ...opts });
        clearTimeout(timer);
        ...
        clearTimeout(timer);
        lastErr = err;
        const retryable = err instanceof LlmRateLimitError || err instanceof LlmTimeoutError || (err instanceof LlmHttpError) || (err && (err.name === 'AbortError' || err.code === 'ECONNRESET'));
        // LlmHttpError retry chi khi 5xx
